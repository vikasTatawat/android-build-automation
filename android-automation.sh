...
S3_BUCKET="##"
CURRENT_DATE="$(date +%d-%m-%Y)/apks/"
EPOCH_TIME=$(date +%s)
FILE_NAME="app-release-"$EPOCH_TIME".apk"
...
# Replace com.pro.CURRENT_PACKAGE_NAME_GIVEN_IN_PROJECT with $PACKAGE_NAME across the project
echo "Replacing package name com.pro.CURRENT_PACKAGE_NAME_GIVEN_IN_PROJECT with com.pro.$PACKAGE_NAME"

grep -rl 'com.pro.CURRENT_PACKAGE_NAME_GIVEN_IN_PROJECT' . | xargs sed -i "s/com\.pro\.CURRENT_PACKAGE_NAME_GIVEN_IN_PROJECT/com.pro.$PACKAGE_NAME/g"
...
APK_FILE=\$(find "$REMOTE_APK_PATH" -name "*.apk" | head -n 1)
